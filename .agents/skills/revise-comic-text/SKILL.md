...
1. Read the human review/request and decide whether it changes meaning or only one locale's wording.
...
5. Do not modify `scene` unless the new caption meaning makes the accepted artwork factually inconsistent. If artwork would need to change, stop and route the visual change through `$generate-scp-comic-images` for the affected panel and then `$finalize-scp-comic`.
6. Validate the YAML:
...
7. Recompose/embed using the already accepted panels; do **not** run imagegen for text-only corrections:
...
8. Run the publish gate:

```bash
python scripts/publish_check.py <id>
```

9. Review warnings. If any language overflows, adjust that language without changing the canonical fact, then rerun the full-language embed and publish check.
10. Report which semantic text changed and confirm that all production languages were updated/validated.
